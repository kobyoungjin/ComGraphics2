	...
	float screenDepth, float screenNear)
{
	...
	if(FAILED(m_device->CreateBlendState(&blendStateDescription, &m_alphaEnabledBlendState)))
	{
		return false;
	}
	...
	// 알파블렌드 상태값을 생성합니다.
	if(FAILED(m_device->CreateBlendState(&blendStateDescription, &m_alphaDisabledBlendState)))
	{
		return false;
	}
